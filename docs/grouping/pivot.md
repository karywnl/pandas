...
!!! connect "Reshaping the summary"
    - `pivot_table()` is [GroupBy](groupby.md) plus a reshape, and equals a group-then-[`unstack`](unstack.md). That group-then-`unstack` step, done by hand, is [multi-level GroupBy](multi-level-groupby.md).
    ...
    - The opposite direction, wide back to long, is [melt](melt.md): pivoting spreads rows into columns, melting folds columns into rows.
